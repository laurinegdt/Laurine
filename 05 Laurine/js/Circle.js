...
// Création d'un gradient radial
    const gradient = this.context.createRadialGradient(0, 0, 0, 0, 0, this.rayon);
 // Couleur intérieure du gradient (centrale)
    gradient.addColorStop(0, "yellow");
// Couleur extérieure du gradient (périphérie)
    gradient.addColorStop(1, this.color);
// Application du gradient en tant que couleur de remplissage
...
// Dessin du cercle
  ...
  definirRayonAleatoire() {
    this.rayonFinal.rayon = Math.random() * 300;
    this.uniteDeTemps1 = 0;
  }
  ...
  move() {
    ...
    // this.angle++;
    // this.x = this.origin.x + Math.cos(this.angle * (Math.PI / 210)) * 100;
    // this.y = this.origin.y + Math.sin(this.angle * (Math.PI / 210)) * 100;
